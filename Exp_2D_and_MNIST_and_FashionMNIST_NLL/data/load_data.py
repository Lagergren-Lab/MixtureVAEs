...
def load_noise(batch_size_tr=128, batch_size_val=128, batch_size_test=128, split_seed=42):
    torch.manual_seed(split_seed)
    x = np.random.beta(1, 1, (10000, 28, 28))
    test = TensorDataset(torch.from_numpy(x), torch.zeros(10000) + 11.)
    test_dataloader = DataLoader(test, batch_size=batch_size_test, shuffle=True)
