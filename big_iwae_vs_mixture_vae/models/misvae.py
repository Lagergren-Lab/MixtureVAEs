...
class MISVAE(VAE):
    def __init__(self, S=2, decoder=None, L=1, device='cuda:0', seed=0,
                 x_dims=784, z_dims=40, beta=1., lr=1e-3, ais=False, No_layers = 1, NLayered = False, h_dim_1 = 300):
        ...
        if ais:
            ...
            self.encoder = EnsembleEncoders(n_dims=x_dims, latent_dims=z_dims, h_dim_1=h_dim_1, h_dim_2=300,
                                            S=S, device=self.device, gated=True, No_layers = No_layers, NLayered=NLayered).to(self.device)


            self.phi = self.encoder.parameters()

            print("num encoder params",sum(p.numel() for p in self.encoder.parameters()))
