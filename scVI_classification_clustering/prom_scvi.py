...
def one_hot(index: torch.Tensor, n_cat: int) -> torch.Tensor:
    ...
    onehot = torch.zeros(index.size(0), n_cat, device='cuda:6')
...
def log_zinb_positive(
    x: torch.Tensor, mu: torch.Tensor, theta: torch.Tensor, pi: torch.Tensor, eps=1e-8, device='cuda:6'
):
    """
    ...
    softplus_pi = F.softplus(-pi).to(device)   #  uses log(sigmoid(x)) = -softplus(-x)
    log_theta_eps = torch.log(theta + eps).to(device)
    log_theta_mu_eps = torch.log(theta + mu + eps).to(device)
    pi_theta_log = -pi + theta * (log_theta_eps - log_theta_mu_eps).to(device)

    case_zero =( F.softplus(pi_theta_log) - softplus_pi ).to(device)
    mul_case_zero = torch.mul((x < eps).type(torch.float32).to(device), case_zero).to(device)

    case_non_zero = (
        -softplus_pi
        + pi_theta_log
        +( x.to(device) * (torch.log(mu + eps).to(device) - log_theta_mu_eps) ).to(device)
        + torch.lgamma(x.to(device) + theta).to(device)
        - torch.lgamma(theta).to(device)
        - torch.lgamma(x.to(device) + 1).to(device)
    ).to(device)
    mul_case_non_zero = torch.mul((x > eps).type(torch.float32).to(device), case_non_zero).to(device)
...
class ZeroInflatedNegativeBinomial(NegativeBinomial):
    ...
    def log_prob(self, value: torch.Tensor) -> torch.Tensor:
        ...
        return log_zinb_positive(value, self.mu, self.theta, self.zi_logits,eps=1e-08, device=self.device)
