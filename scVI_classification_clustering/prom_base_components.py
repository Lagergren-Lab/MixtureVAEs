                ...
                if layer is not None:
                    if isinstance(layer, nn.BatchNorm1d):
                        if x.dim() == 3: #> 2
                            ...
                            x = torch.cat(
                                [(layer(slice_x)) for slice_x in x], dim=0
                            )
                        else:
                            x = layer(x)
                    else:
...
class EnsembleEncoders(nn.Module):
    def __init__(self,
        ...
        self.encoders = nn.ModuleList([
            Encoder(n_input, n_output, n_cat_list=n_cat_list, n_layers=n_layers, n_hidden=n_hidden,
                    dropout_rate=dropout_rate, distribution =distribution, var_eps=var_eps,
                    var_activation = var_activation).to(self.device) for _ in range(self.S)]
        )
    ...
    def forward(self, x):
        mu = torch.zeros((x.size(0), self.S, self.n_output), device=self.device) #, device=self.device
